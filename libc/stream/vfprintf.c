/* Foreign file. From Sortix OS. Adapted to YAX by Min4Builder */
/*
 ...
 *
 * stream/vfprintf.c
 * Formats text and outputs it to a stream.
 */
	...
	while ( *format != '\0' )
	{
		...
		print_c:
			...
			while ( format[amount] && format[amount] != '%' )
				amount++;
			if ( fwrite(format, 1, amount, stream) != amount )
				return -1;
			format += amount;
			written += amount;
			continue;
		     ...
		     *format == 'p' )
		{
			...
			if ( use_left_pad )
			{
				...
				{
					if ( fwrite(" ", 1, 1, stream) != 1 )
						return -1;
					written++;
				}
			}
			if ( fwrite(prefix, 1, prefix_length, stream) != prefix_length )
				return -1;
			...
			if ( fwrite(output, 1, output_length, stream) != output_length )
				return -1;
		...
		else if ( *format == 's' )
		{
			...
			if ( fwrite(string, 1, string_length, stream) != string_length )
				return -1;
