...
if ! test -f .configured; then
	../gcc-$ver/configure --target="$YAX_ARCH-$TARGET" --prefix="" --oldincludedir="/include" --disable-shared --enable-languages=c --disable-nls --without-headers --disable-lto --with-sysroot="$YAX_PREFIX"
	: > .configured
fi

mkdir -p "$YAX_PREFIX/include"
